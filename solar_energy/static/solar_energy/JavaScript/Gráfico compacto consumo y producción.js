...
for (var i = 0; i < datos_consumo.length; i++) {
  var mes = datos_consumo[i].Mes;
  var consumo = datos_consumo[i].CONSUMO_Wh;
  var produc = datos_consumo[i].Produccion;
...
function myResponsiveComponent1(container, props){
  ...
  let yAxisG = g.selectAll('.y-axis1').data([null]);
    ...
    // Crea las barras
  let grafico = g.selectAll('.bar1').data(consumos);
  ...
  let grafico2 = g.selectAll('.bar2').data(produccion);
...
function render1(){
  myResponsiveComponent1(d3.select("#Bar_Chart_consumo_produccion"), {
